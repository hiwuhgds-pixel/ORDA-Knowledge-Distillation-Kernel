...
if HAS_TRITON:
    @triton.jit
    def _precomputed_student_ce_kl_fwdbwd_kernel(
        S_ptr, S_stride,
        T_ptr, T_stride,
        Y_ptr, Y_stride,
        L_ptr,
...
def _precomputed_student_fwdbwd(
    ...
    *,
    ...
    if not HAS_TRITON:
        raise RuntimeError("_precomputed_student_fwdbwd requires Triton and CUDA/HIP tensors.")
...
# ── Autograd Function ─────────────────────────────────────────────────────────
class PrecomputedCEFunction(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        student_hidden,
        ...
        if float(teacher_ce_weight) != 0.0:
            raise ValueError("PrecomputedCEFunction requires teacher_ce_weight=0.0")
        ...
        if logits_teacher is not None:
            ...
            raise ValueError(
                "PrecomputedCEFunction requires logits_teacher or (teacher_hidden, teacher_weight)"
            )
...
def precomputed_distillation_loss(
    ...
    if not HAS_TRITON:
        raise RuntimeError(
            "precomputed_distillation_loss requires Triton and CUDA/HIP tensors. "
    ...
    if logits_teacher is not None:
        if logits_teacher.shape != (BT, V):
            raise ValueError(f"logits_teacher must have shape ({BT}, {V}), got {tuple(logits_teacher.shape)}")
        if logits_teacher.requires_grad:
            raise ValueError(
                "precomputed_distillation_loss requires logits_teacher without gradients; "
                "use SeparateTeacher when the teacher path needs gradients"
            )
    elif has_hidden_weight:
        if teacher_hidden.ndim != 2 or teacher_hidden.shape[0] != BT:
            raise ValueError(
                f"teacher_hidden must have shape (BT, D_t), got {tuple(teacher_hidden.shape)}"
            )
        if teacher_weight.shape != (V, teacher_hidden.shape[1]):
            raise ValueError(
                f"teacher_weight must have shape ({V}, {teacher_hidden.shape[1]}), "
                f"got {tuple(teacher_weight.shape)}"
            )
        if teacher_hidden.requires_grad or teacher_weight.requires_grad:
            raise ValueError(
                "teacher_hidden and teacher_weight must not require gradients"
            )
    else:
        raise ValueError(
            "precomputed_distillation_loss requires logits_teacher or (teacher_hidden, teacher_weight)"
        )
    ...
    if _teacher_ce_weight != 0.0:
        raise ValueError("Triton precomputed_distillation_loss requires teacher_ce_weight=0.0")
    ...
    device_tensors = [labels, weight]
    if logits_teacher is not None:
        device_tensors.append(logits_teacher)
    if teacher_hidden is not None:
        device_tensors.extend([teacher_hidden, teacher_weight])
    if any(t.device != device for t in device_tensors):
        raise ValueError("all tensors must be on the same device as student_hidden")
    ...
    return PrecomputedCEFunction.apply(
        student_hidden,
        weight,
        logits_teacher,
        labels,
