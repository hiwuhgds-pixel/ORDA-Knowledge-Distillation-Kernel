...
def main() -> None:
    parser = argparse.ArgumentParser(description="CE-only fused kernel benchmark")
    add_common_benchmark_args(parser, default_configs=CANONICAL_T4_CONFIGS["ce_only"])
    parser.add_argument("--no-compile", action="store_true",
                        help="Disable torch.compile on the PyTorch baseline loss path.")
    args = parser.parse_args()
    validate_positive_timing_args(args)

    runtime = require_cuda_kernel_or_skip("bench_ce_only", args)
    torch = runtime.torch
    ...
    def _pytorch_ce_eager(hs, ht, weight, target):
        logits_s = F.linear(hs, weight)
        logits_t = F.linear(ht, weight)
        return F.cross_entropy(logits_s, target) + F.cross_entropy(logits_t, target)

    pytorch_ce = compile_pytorch_loss(torch, _pytorch_ce_eager, enabled=loss_compile_enabled)

    print_table_header(
        f"Orda CE-only benchmark (repeats={args.repeats}, steps={args.steps}, "
    ...
    for batch, seq in args.configs:
        ...
        def native_step():
            ...
            loss = pytorch_ce(hs, ht, weight, target)
            loss.backward()

        def orda_step():
            hs.grad = ht.grad = weight.grad = None
            loss, *_ = distill_cross_entropy(hs, ht, weight, target, lambda_student=1.0, kl_weight=0.0)
            loss.backward()

        for method, step in [("PyTorch CE(student+teacher)", native_step), ("Orda fused CE", orda_step)]:
            try:
    ...
    write_artifacts(
        rows,
        collect_metadata(torch, args, "bench_ce_only"),
