...
def import_torch():
    try:
        ...
        raise RuntimeError(
            "PyTorch is required. Install the package environment with "
            "`python -m pip install -e .` before running tests."
        ) from exc
...
def import_orda():
    try:
        ...
        raise RuntimeError(
            "orda_ce_kernel is not importable. Run commands from an environment "
            "where the package has been installed, e.g. `python -m pip install -e .`."
        ) from exc
...
def exit_code_from_exception(exc: Exception) -> int:
