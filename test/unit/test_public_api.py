...
def test_default_auto_backend_on_cpu_matches_explicit_torch_backend():
    hs, ht, weight, labels = _valid_cpu_inputs()
    teacher = orda_ce_kernel.TiedTeacher(ht)

    out_auto = orda_ce_kernel.distillation_loss(
        hs,
        weight,
        labels,
        teacher,
    ...
    out_torch = orda_ce_kernel.distillation_loss(
        hs,
        weight,
        labels,
        teacher,
    ...
    assert torch.allclose(out_auto.loss, out_torch.loss)
    assert torch.allclose(out_auto.student_ce, out_torch.student_ce)
    assert torch.allclose(out_auto.teacher_ce, out_torch.teacher_ce)
    assert torch.allclose(out_auto.kl, out_torch.kl)
...
def test_precomputed_teacher_accepts_hidden_weight_cache_in_torch_backend():
    hs = torch.randn(5, 4, requires_grad=True)
    ...
    out = orda_ce_kernel.distillation_loss(
        ...
        weight,
        labels,
        orda_ce_kernel.PrecomputedTeacher(
...
def test_precomputed_teacher_rejects_ambiguous_or_grad_cache():
    logits = torch.randn(4, 16)
    teacher_hidden = torch.randn(4, 8)
    teacher_weight = torch.randn(16, 8)
    with pytest.raises(ValueError, match="logits or"):
        orda_ce_kernel.PrecomputedTeacher(
            logits=logits,
            teacher_hidden=teacher_hidden,
            teacher_weight=teacher_weight,
    ...
    with pytest.raises(ValueError, match="both be provided"):
        orda_ce_kernel.PrecomputedTeacher(teacher_hidden=teacher_hidden)

    hs, _ht, weight, labels = _valid_cpu_inputs()
    bad_logits = torch.randn(labels.shape[0], weight.shape[0], requires_grad=True)
    with pytest.raises(ValueError, match="must not require gradients"):
        orda_ce_kernel.distillation_loss(
            hs,
            weight,
            labels,
            orda_ce_kernel.PrecomputedTeacher(logits=bad_logits),
            backend="torch",
        )

    bad_hidden = teacher_hidden.requires_grad_(True)
    with pytest.raises(ValueError, match="must not require gradients"):
        orda_ce_kernel.distillation_loss(
            hs,
            weight,
...
def test_kernel_config_profile_and_validation_contract():
    debug = public_api._resolve_profile("debug")
    assert debug.fp32_grad_weight_accumulation is True
    balanced = public_api._resolve_profile("balanced")
    assert balanced.fp32_grad_weight_accumulation is False
    merged = public_api._resolve_config("debug", orda_ce_kernel.KernelConfig(chunk_size=7))
    assert merged.fp32_grad_weight_accumulation is True
    assert merged.chunk_size == 7
    override = public_api._resolve_config(
        "debug",
    ...
    assert override.fp32_grad_weight_accumulation is False

    with pytest.raises(ValueError, match="profile"):
        public_api._resolve_profile("fast")
    ...
    with pytest.raises(ValueError, match="max_chunks"):
        orda_ce_kernel.distillation_loss(
            ...
            labels,
            orda_ce_kernel.TiedTeacher(ht),
            config=orda_ce_kernel.KernelConfig(max_chunks=0),
        )
    with pytest.raises(ValueError, match="num_chunks"):
        orda_ce_kernel.distillation_loss(
            ...
            labels,
            orda_ce_kernel.TiedTeacher(ht),
            config=orda_ce_kernel.KernelConfig(num_chunks=0),
        )
    with pytest.raises(ValueError, match="mutually exclusive"):
        orda_ce_kernel.distillation_loss(
            ...
            labels,
            orda_ce_kernel.TiedTeacher(ht),
            config=orda_ce_kernel.KernelConfig(chunk_size=2, num_chunks=2),
        )
...
def test_bool_labels_are_rejected_before_dispatch():
    ...
    with pytest.raises(ValueError, match="integer class-index|torch.long"):
        orda_ce_kernel.distillation_loss(
            hs,
            weight,
            labels,
...
def _manual_tied_reference(hs, ht, weight, labels, *, kl_weight, kl_temperature, reduction):
    ...
    if reduction == "mean":
        ...
        ce_t = ce_t_all.sum()
        kl = kl_all.sum()
    return ce_s + ce_t + kl_weight * kl, ce_s, ce_t, kl
